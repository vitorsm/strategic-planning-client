...
export interface CreateUpdateEntityPageProps<T extends GenericEntity> {
...
export const CreateUpdateEntityPage = <T extends GenericEntity>({
  children,
  setPageTitle,
  ...
  const returnPage = queryParams.get('return_page');
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (result) {
      // If we're creating a new entity and a return_page was provided, navigate there.
      // Otherwise, fall back to the default back address.
      if (returnPage) {
        navigate(returnPage, { state: { refresh: true } });
      } else {
  ...
  return (
    <DetailsPageWrap $hasBottomMenu={showActionButtons}>
